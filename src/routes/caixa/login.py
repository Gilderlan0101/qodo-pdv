...
from src.core.cache import client  # Cliente Redis
...
class LoginCheckout:
    ...
    def _register_routes(self):
        ...
        async def login_and_open(
            ...
            # 🔍 DEBUG: Mostra informações da senha armazenada
            LOGGER.info(
                f'🔍 DIAGNÓSTICO - Senha DB: {employee.senha} | Tipo: {type(employee.senha)}'
            )
            LOGGER.info(
                f'🔍 DIAGNÓSTICO - Senha input: {user.password} | Tipo: {type(user.password)}'
            )
